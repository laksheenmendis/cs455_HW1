...
public class StatisticsCollectorAndDisplay {
    ...
    public static void printTrafficSummary(List<OverlayNodeReportsTrafficSummary> trafficSummaries) {
        ...
        printLineSeperator3();
        String s = String.format("|%-10s|%-12s|%-17s|%-16s|%-16s|%-30s|",empty, pckSent, pckRcvd, pckRlyd, sumSentH, sumRcvdH);
        System.out.println(s);
        printLineSeperator3();
        ...
        for(OverlayNodeReportsTrafficSummary trafficSummary : trafficSummaries)
        {
            ...
            String s1 = String.format("|%10s|%,12d|%,17d|%,16d|%,16d|%,30d|",node, trafficSummary.getTotalPacketsSent(), trafficSummary.getTotalPacketsReceived(), trafficSummary.getTotalPacketsRelayed(), trafficSummary.getSumOfPacketDataSent(), trafficSummary.getSumOfPacketDataReceived());
            System.out.println(s1);
            printLineSeperator3();
        }

        String sumS = "Sum";
        String s2 = String.format("|%10s|%,12d|%,17d|%,16d|%,16d|%,30d|",sumS, totPckSent, totPckRcvd, totPckRlyd, totalSentSummation, totalRcvdSummation);
        System.out.println(s2);
        printLineSeperator3();
